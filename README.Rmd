...
The main features of this package are:
 ...
 - Some key functions with too many iterations implemented in C++, therefor could have better performance
...
You can provide your `sf` class object and specify the `from` and `to` coordicates as arguments. (Please note this function currently supports only the geometry types: `POINT`, `LINESTRING`, `POLYGON`, `MULTIPOINT`, `MULTILINESTRING` and `MULTIPOLYGON`)

This example illustrates the adjustment method. The `example_car_trace` represents som sample points from car trace with coordinates in `gcj02.` The figure demonstrates that, without adjustment, the car trace log deviates from the roads, which could not be right. 
